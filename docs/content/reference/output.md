...
describes what users get. Pick a format with `-o`, or let huobi choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
huobi <command> -o table   # aligned columns for reading
huobi <command> -o jsonl   # one JSON object per line, for piping
huobi <command> -o json    # a single JSON array
huobi <command> -o csv     # spreadsheet friendly
huobi <command> -o tsv     # tab-separated
huobi <command> -o url     # just the URL column
huobi <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
huobi <command> --fields id,title,url
```
...
```bash
huobi <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
huobi <command>            # a table, because this is a terminal
huobi <command> | wc -l    # JSONL, because this is a pipe
```
